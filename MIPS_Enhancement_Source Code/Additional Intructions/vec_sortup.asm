...
#
# Description: 		Code implementing the instruction 'vec_sortup d, a' into MIPS architecture. D and A are 8-byte vectors with 2-byte wide elements. The upper and lower halves of each vector are split between two registers. The instruction sorts the elements in A until the entire vector is in increasing numerical order. The sorted result is stored in D.
#
# Register useage: 	$t0 - contains the upper half of vector A
#		  	$t1 - contains the lower half of vector A
#			$t2 - contains an element of A to be compared with the following element, then swapped if not less than
#		  	$t3 - contains an element of A to be compared with the previous element, then swapped if less than
...
L1:	add		$t9, $zero, $zero	# sets the swap checker back to zero
	...
	sll		$t3, $s0, 8				
	srl		$t3, $t3, 24
	slt		$a0, $t2, $t3		# checks if the element in t2 is less than t3's, branches to L2 if so
	bne		$a0, $zero, L2
...
L2:	sll		$t2, $s0, 8		# shift t2 and t3 left then right to gain the second and third elements
	...
	srl		$t2, $t2, 24			
	srl 		$t3, $t3, 24
	slt		$a0, $t2, $t3		# checks if the element in t2 is less than t3's, branches to L3 if so
	bne		$a0, $zero, L3
...
L3:	sll		$t2, $s0, 16		# shift t2 and t3 left then right to gain the third and fourth elements
	...
	srl		$t2, $t2, 24			
	srl 		$t3, $t3, 24
	slt		$a0, $t2, $t3		# checks if the element in t2 is less than t3's, branches to L4 if so
	bne		$a0, $zero, L4
...
L4:	sll		$t2, $s0, 24		# shift t2 and t3 left then right to gain the fourth and fifth elements
	srl		$t2, $t2, 24			
	srl 		$t3, $s1, 24
	slt		$a0, $t2, $t3		# checks if the element in t2 is less than t3's, branches to L5 if so
	bne		$a0, $zero, L5
...
L5:	srl		$t2, $s1, 24		# shift t2 and t3 left then right to gain the fifth and sixth elements
	sll		$t3, $s1, 8		
	srl		$t3, $t3, 24
	slt		$a0, $t2, $t3		# checks if the element in t2 is less than t3's, branches to L6 if so
	bne		$a0, $zero, L6
...
L6:	sll		$t2, $s1, 8		# shift t2 and t3 left then right to gain the sixth and seventh elements
	...
	srl		$t2, $t2, 24			
	srl 		$t3, $t3, 24
	slt		$a0, $t2, $t3		# checks if the element in t2 is less than t3's, branches to L7 if so
	bne		$a0, $zero, L7
