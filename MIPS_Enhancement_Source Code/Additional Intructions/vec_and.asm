...
#
# Description: 		Code implementing the instruction 'vec_and d, a, b' into MIPS architecture. D, A, and B are 8-byte vectors with 2-byte wide elements. The upper and lower halves of each vector are split between two registers. The instruction and's each element from A and B one at a time and stores it into the corresponding element in vector D.
#
...
#  		   	$t4 - contains vector A's two bytes that are being and'd, then receives the result to be stored into vector D. Set to 0xff if the result causes overflow
