...
main:	li		$t0, 0x233C475D		# load the lower half of vector A into t0
	...
	beq		$t0, $t2, LoTest	# checks if the upper halves of A and B are equal and branches to LoTest if so. Exits if not
	j		exit
	
LoTest: beq		$t1, $t3, Target	# checks if the lower halves of A and B are equal and branches to Target if so. Exits if not
