...
#
# Description: 		Code implementing the instruction 'vec_mule d, a, b' into MIPS architecture. A, B, and C are 8-byte vectors with 2-byte wide elements, and D has 4-byte wide elements. The upper and lower halves of each vector are split between two registers. The instruction multiplies the even-numbered element positions of A and B and stores the 16-bit product into vector D.
#
...
#			$t4 - contains vector A's two bytes that are being multiplied, handling the even-numbered element positions. It holds the product and stores it into vector D.
#  		  	$t5 - contains vector B's two bytes that are being multiplied, handling the even-numbered element positions.
